...
namespace ReportItemReader.XML
{
    ...
    public class XMLReportItemReader : IReportItemReader
    {
        ...
        private static IReportComponent ParseToReportComponent(XmlNode node)
        {
            ...
            var typeComp = node.HasAttributes() ? node.GetAttributeValue("type") : node.Name;

            reportComponent.TypeOfComponent = Enum.TryParse(typeComp, out ReportComponentType type) ? type : ReportComponentType.Default;
